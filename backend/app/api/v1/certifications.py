...
async def generate_certification_pack(
    ...
    sections.append({
        ...
        "content": {
            ...
            "data_classification": use_case.data_classification.value if use_case.data_classification else None,
    ...
    if payload.include_monitoring_plan:
        result = await db.execute(
            ...
            .where(MonitoringPlan.use_case_id == payload.use_case_id)
        )
    ...
    return CertificationPackResponse(
        pack_id=f"CP-{payload.use_case_id[:8]}-{datetime.now(timezone.utc).strftime('%Y%m%d')}",
...
async def generate_certification_pack_pdf_endpoint(
    ...
    filename = f"CertPack_{pack_data['pack_id']}_{datetime.now(timezone.utc).strftime('%Y%m%d')}.pdf"
    return Response(
        content=pdf_bytes,
